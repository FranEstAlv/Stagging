...
class CanalFacade:
    ...
    modulo necesite saber el chat_id de antemano. Mismo espiritu que
    refe_command de ALFA-1 (imagen + texto especifico + botones, publicado
    a la vez en un canal Y en un grupo principal, cada uno con su propio
    boton y su propio resultado de exito/fallo -- ver
    alfa1/samaritan/core.py linea ~10182) pero resuelto por configuracion
    editable (chat_id, boton, plantilla, activo, periodicidad) por
    destino, no hardcodeado en el modulo.
    ...
    entre todos los destinos activos del modulo, igual que refe_command."""
    ...
    async def publicar_a_todos(
        ...
        """Publica en TODOS los destinos activos del modulo a la vez (igual
        que refe_command: canal + grupo principal simultaneo). Ningun
        destino caido tumba a los demas -- cada uno se intenta por
