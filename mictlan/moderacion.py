...
# (samaritan/services/global_moderation.py: blacklist + apply_global_ban +
# block_if_global_blacklisted), reescrito de cero con las convenciones de
# Mictlan -- ver la regla prohibitiva de espejo en CLAUDE.md. "Banear" acá
# significa EXCLUSIVAMENTE expulsar (kick) de todos los grupos/canales
# gestionados (tabla grupos) + guardar un reporte (motivo + foto) para que
# un admin sepa por que no debe volver a aceptar a ese usuario -- nunca se
# confunde con la futura expulsion automatica por membresia vencida (esa
# es solo por tiempo, sin motivo, sin blacklist).
