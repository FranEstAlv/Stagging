    ...
    async with pool.acquire() as conn:
        filas = await conn.fetch(
            ...
            SELECT u.user_id, u.username, u.rol,
                   (b.user_id IS NOT NULL) AS baneado
            FROM usuarios u
            LEFT JOIN blacklist b ON b.user_id = u.user_id AND b.activo = true
...
async def _vista_detalle(user_id: int, pagina_origen: int = 0, nota: str | None = None) -> tuple[str, InlineKeyboardMarkup]:
    ...
    if membresia and membresia["activa"] and membresia["fin"]:
        lineas.append(f"Membresía: activa hasta {membresia['fin'].strftime('%Y-%m-%d')}")
    elif membresia and membresia["fin"]:
        lineas.append(f"Membresía: vencida ({membresia['fin'].strftime('%Y-%m-%d')})")
    else:
        lineas.append("Membresía: sin membresía")
    if baneo:
        lineas.append(f"Estado: ⛔ baneado ({baneo['creado_en'].strftime('%Y-%m-%d')})")
    else:
