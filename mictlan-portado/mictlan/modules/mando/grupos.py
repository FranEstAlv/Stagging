...
# (modules/grupos.py, ChatMemberHandler sobre my_chat_member). Esto solo
# agrega la UI para listar/activar/desactivar lo que ya se detecto --
# nunca se borra un grupo de la tabla desde aca, no forma parte del
# contrato de esta seccion.
...
async def _vista_detalle(chat_id: int, pagina_origen: int = 0, nota: str | None = None) -> tuple[str, InlineKeyboardMarkup]:
    ...
    lineas = [
        ...
        f"Agregado: {g['agregado_en'].strftime('%Y-%m-%d %H:%M')}",
    ...
    if not g["principal"]:
        ...
        # /mando -- los grupos/canales secundarios usan el comando
        # /canales (mictlan/modules/canales.py), no este panel.
