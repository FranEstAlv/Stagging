...
# Modo de mantenimiento -- ver "Modo de mantenimiento (plan)" en
# CLAUDE.md. Por diseno esto es SOLO un estado consultable: no detiene el
# bot, no cierra grupos, no expulsa a nadie. Sirve para que una fase
# futura (heartbeat/chequeo de salud, todavia no existe en Mictlan) pueda
# distinguir una pausa deliberada de una caida real vía
# esta_en_mantenimiento(). 'hasta'/'iniciado_en'/'finalizado_en' son
# TIMESTAMPTZ reales, sin parsear ni formatear texto a mano.


def tiempo_restante(hasta: datetime | None) -> str:
    if not hasta:
        return "sin límite"
    segundos = int((hasta - datetime.now(timezone.utc)).total_seconds())
    if segundos <= 0:
        return "vencido"
    minutos = segundos // 60
    if minutos < 60:
