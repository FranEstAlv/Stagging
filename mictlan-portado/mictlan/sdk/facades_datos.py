...
class AlmacenPropioFacade:
    """Expuesto como contexto.datos.db -- persistencia propia y privada por
    modulo (mictlan/almacen_modulos.py), async de verdad (aiosqlite, nunca
    bloquea el event loop), para que un modulo pueda recordar datos de sus
    usuarios entre restarts sin necesitar acceso a la base Postgres
    principal. Requiere el scope 'datos.escribir_propio' -- deliberadamente
    distinto de 'datos.leer_propio' (ese es solo para los archivos de
    referencia que un admin coloca a mano, ver DatosFacade) porque son dos
    niveles de privilegio distintos: leer un CSV de catalogo no es lo
    mismo que poder crear/escribir tablas propias.

    Nota de nombres: NUNCA se llamo 'contexto.db' ni el scope 'db.propio'
    -- 'db.' esta en SCOPES_PREFIJOS_PELIGROSOS (bloqueado a proposito) --
    de ahi el nombre real 'datos.escribir_propio', dentro del mismo
    namespace que los otros scopes de datos."""
...
class DatosFacade:
    ...
    SQLite de referencia, sin importar el tamano. Dos scopes separados
    porque son dos niveles de privilegio distintos:
