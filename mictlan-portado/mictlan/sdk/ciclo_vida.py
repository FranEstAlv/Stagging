...
# Guardia contra reimportacion repetida. Acotado por la cantidad de
# modulos instalados (finito, controlado por un admin), no por input de
# usuario.
...
async def sincronizar_registro() -> list[str]:
    ...
    que todavia no tenga fila -- SIEMPRE inactivo por defecto (activo=false),
...
async def alternar_origen(module_id: str) -> str:
    ...
    modulos ya se consideran 'graduados' y candidatos a portarse a mano a
    mictlan/modules/. Devuelve el origen nuevo."""
    pool = db.get_pool()
    async with pool.acquire() as conn:
